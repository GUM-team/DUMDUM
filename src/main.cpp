...
CBigNum bnProofOfWorkLimit(~uint256(0) >> 20);      // DIF IS DUMDUM
CBigNum bnProofOfStakeLimit(~uint256(0) >> 20);
CBigNum bnProofOfWorkLimitTestNet(~uint256(0) >> 16);

unsigned int nTargetSpacing     = 60;               // 60 seconds DUMDUM
unsigned int nStakeMinAge       = 1 * 60 * 60;      // 1 hour min stake age DUMDUM
unsigned int nStakeMaxAge       = 20 * 60 * 60;      // 20 hours max stake age DUMDUM
unsigned int nModifierInterval  = 10 * 60;          // time to elapse before new modifier is computed DUMDUM
...
CMedianFilter<int> cPeerBlockCounts(5, 0); // Amount of blocks that other nodes claim to have DUMDUM
...
const string strMessageMagic = "DUMDUM Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	if(pindexBest->nHeight < 1)
    {
        ...
        else if(pindexBest->nHeight < 100)  //9900 GUM DUMDUM
    {
        nSubsidy = 100 * COIN;
    }
        else if(pindexBest->nHeight < 1001)  //720000 GUM DUMDUM
    {
        nSubsidy = 800 * COIN;
    }
        else if(pindexBest->nHeight < 2001)  //400000 GUM DUMDUM
    {
        nSubsidy = 400 * COIN;
    }
        else if(pindexBest->nHeight < 3001)  //1200000 GUM DUMDUM
    {
        nSubsidy = 1200 * COIN;
    }
        else if(pindexBest->nHeight < 4001)  //600000 GUM DUMDUM
    {
        nSubsidy = 600 * COIN;
    }
        else if(pindexBest->nHeight < 5001)  //1000000 GUM DUMDUM
    {
		nSubsidy = 1000 * COIN; 
    }
...
const int YEARLY_BLOCKCOUNT = 525948;
    ...
    int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
    {
        ...
        int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);  //IF ELSE 9% DUMDUM
            ...
            else if(pindexBest->nHeight < 5001)
        {
            ...
            else if(pindexBest->nHeight < 10001)  // 500000 GUM DUMDUM
        {
            nSubsidy = 100 * COIN;
        }
            else if(pindexBest->nHeight < 11001)  // 800000 GUM DUMDUM
        {
            nSubsidy = 800 * COIN;
        }
            else if(pindexBest->nHeight > 15000) // 9% FOREVER DUMDUM
        {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DUMDUM", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "2015 DUMDUM GUMGUM You DUMDUM me want GUMGUM 2015";
        CTransaction txNew;
        txNew.nTime = 1430210071;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1430210071;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1403619;
        if(fTestNet)
        {
            block.nNonce   = 0;
            block.nTime    = 1430210071;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xa39108772cb275d2413b8ff317b376fe584f0b0a5eb41071c765428681529cb4"));
        block.print();
